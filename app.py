...
ヒアリング完了時には、これまでのヒアリング内容全体を要約し、以下の追加項目も必ず含めてください。
{
...
- あなたの応答は、常にJSON形式で、`aiResponse` (ユーザーへの表示メッセージ、マークダウンやHTMLタグは含めないプレーンテキスト) と `extractedInfo` (抽出情報オブジェクト) を含むようにしてください。
- {SYSTEM_PROMPT_JSON_FORMAT_GUIDANCE}
